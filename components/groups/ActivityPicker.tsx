import { Check, Plus, X } from "lucide-react-native";
import { useState } from "react";
import { Pressable, Text, TextInput, View } from "react-native";

import { Chip } from "@/components/ui/Chip";
import { colors } from "@/constants/colors";
import { DEFAULT_SPORTS, getSportIcon, normalizeSport } from "@/lib/sports";

type Props = {
  value: string[];
  onChange: (value: string[]) => void;
  /** Sports proposés par défaut (chips toujours visibles). */
  suggestions?: readonly string[];
};

/**
 * Sélecteur d'activités DA : chips des sports proposés + sports ajoutés, toggle ; bouton
 * « Ajouter » ouvrant un champ texte libre (n'importe quel sport). L'icône de chaque chip est
 * déterminée automatiquement via `lib/sports.ts` (fallback générique). Stocke des **noms**.
 */
export function ActivityPicker({ value, onChange, suggestions = DEFAULT_SPORTS }: Props) {
  const [adding, setAdding] = useState(false);
  const [draft, setDraft] = useState("");

  // Liste affichée = suggestions + sports custom, dédupliqués (insensible accents/casse).
  const display: string[] = [];
  const seen = new Set<string>();
  for (const name of [...suggestions, ...value]) {
    const key = normalizeSport(name);
    if (!seen.has(key)) {
      seen.add(key);
      display.push(name);
    }
  }

  const isSelected = (name: string) =>
    value.some((v) => normalizeSport(v) === normalizeSport(name));

  const toggle = (name: string) => {
    const key = normalizeSport(name);
    if (isSelected(name)) onChange(value.filter((v) => normalizeSport(v) !== key));
    else onChange([...value, name]);
  };

  const addCustom = () => {
    const t = draft.trim();
    if (!t) {
      setAdding(false);
      return;
    }
    const key = normalizeSport(t);
    if (!value.some((v) => normalizeSport(v) === key)) {
      const canonical = suggestions.find((s) => normalizeSport(s) === key) ?? t;
      onChange([...value, canonical]);
    }
    setDraft("");
    setAdding(false);
  };

  return (
    <View className="gap-2.5">
      <View className="flex-row flex-wrap gap-2">
        {display.map((name) => (
          <Chip
            key={name}
            label={name}
            icon={getSportIcon(name)}
            selected={isSelected(name)}
            onPress={() => toggle(name)}
          />
        ))}

        {/* Chip « Ajouter » (pointillé) */}
        <Pressable
          onPress={() => setAdding((a) => !a)}
          className="flex-row items-center gap-2 rounded-chip border border-dashed border-line-2 px-3.5 py-[9px] active:opacity-80"
          style={adding ? { backgroundColor: colors.coralSoft, borderColor: "rgba(255,106,69,0.4)" } : undefined}
        >
          <Plus size={16} color={adding ? colors.coral : colors.creamDim} />
          <Text
            className="font-body-semibold text-[13px]"
            style={{ color: adding ? colors.coral : colors.creamDim }}
          >
            Ajouter
          </Text>
        </Pressable>
      </View>

      {adding ? (
        <View className="flex-row items-center gap-2">
          <TextInput
            value={draft}
            onChangeText={setDraft}
            onSubmitEditing={addCustom}
            autoFocus
            placeholder="Ex. Escalade, Boxe…"
            placeholderTextColor="rgba(183,161,139,0.5)"
            returnKeyType="done"
            className="flex-1 rounded-input border border-line bg-surface px-3.5 py-3 font-body text-[14px] text-cream"
          />
          <Pressable
            onPress={addCustom}
            className="h-[46px] w-[48px] items-center justify-center rounded-input bg-coral active:opacity-80"
          >
            <Check size={20} color={colors.onCoral} strokeWidth={2.6} />
          </Pressable>
          <Pressable
            onPress={() => {
              setDraft("");
              setAdding(false);
            }}
            className="h-[46px] w-[46px] items-center justify-center rounded-input border border-line bg-surface active:opacity-80"
          >
            <X size={18} color={colors.creamDim} />
          </Pressable>
        </View>
      ) : null}
    </View>
  );
}
